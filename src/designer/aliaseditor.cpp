    ...
    for (i = 0; i < langCount; i++) {
        ...
        for (j = 0; j < n; j++) {
            ...
            if (langtag == ac->attr(alias, mda_tag)) {
                tAliases->setItem(i, 0, new QTableWidgetItem(ac->attr(alias, mda_name)));
            }
        }
    }
}
...
void aAliasEditor::updateMD()
{
    ...
    for (i = 0; i < tAliases->rowCount(); i++) {
        QTableWidgetItem *item = tAliases->item(i, 0);
        if (item && !item->text().isEmpty()) {
            alias = ac->insert(obj, md_alias, item->text(), -1);

            QTableWidgetItem *headerItem = tAliases->verticalHeaderItem(i);
            if (headerItem)
                ac->setAttr(alias, mda_tag, headerItem->text());
        }
    }
}
